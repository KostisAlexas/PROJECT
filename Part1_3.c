...
int binary_search(time_t target_date, int *found_indexes);
int interpolation_search(time_t target_date, int *found_indexes);
...
int main() {
    ...
    while (1) {
        ...
        char date[11];
        printf("Enter a date (ex: 01/01/2015): ");
        scanf("%10s", date);
        ...
        if (choice == 1) {
            num_found = binary_search(target_date, found_indexes);
        } else if (choice == 2) {
            num_found = interpolation_search(target_date, found_indexes);
        } else {
...
// Function to perform binary search for a target date
int binary_search(time_t target_date, int *found_indexes) {
int left = 0;
...
while (left <= right) {
    ...
    // Check if the target date is at the middle position
    if (target_date == records[mid].record.date) {
        found_indexes[num_found++] = mid;

        // Check for other records with the same date in the left and right of the middle position
        int temp = mid - 1;
        ...
        break;
    } else if (target_date < records[mid].record.date) {
        right = mid - 1;
    } else {
        left = mid + 1;
    }
...
// Function to perform interpolation search for a target date
int interpolation_search(time_t target_date, int *found_indexes) {
int left = 0;
int right = NUM_RECORDS - 1;
int num_found = 0;

// Continue searching while the target date is within the search range
while (left <= right && target_date >= records[left].record.date && target_date <= records[right].record.date) {
    if (left == right) {
        if (records[left].record.date == target_date) {
            found_indexes[num_found++] = left;
        }
        break;
    }

    // Calculate the estimated position of the target date
    int pos = left + (((double)(right - left) / (records[right].record.date - records[left].record.date)) * (target_date - records[left].record.date));

    // Check if the target date is at the estimated position
    if (target_date == records[pos].record.date) {
        found_indexes[num_found++] = pos;

        // Check for other records with the same date in the left and right of the estimated position
        int temp = pos - 1;
        while (temp >= 0 && records[temp].record.date == target_date) {
            found_indexes[num_found++] = temp;
            temp--;
        ...
        break;
    } else if (target_date < records[pos].record.date) {
        right = pos - 1;
    } else {
        left = pos + 1;
    }
